...
const FileCompressIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg fill="currentColor" viewBox="0 0 24 24" {...props}>
    <path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm-1 7V3.5L18.5 9H13v5.5zm-1 9l-3-3h2v-3h2v3h2l-3 3z" />
  </svg>
);
...
type CompressedState = { name: string; size: number } | null;
...
export default function CompressPage() {
  ...
  const [originalFile, setOriginalFile] = useState<FileState>(null);
  const [compressedFile, setCompressedFile] = useState<CompressedState>(null);
  ...
  const getQuality = (level: typeof compressionLevel) => {
    if (level === "extreme") return 50;
    if (level === "less") return 85;
    return 70;
  };

  const compressPdf = async (fileId: string, name: string, originalSize: number) => {
    setIsCompressing(true);
    setError(null);
    try {
      const payload = {
        quality: getQuality(compressionLevel),
      };

      const res = await fetch(`${API_URL}/api/files/${fileId}/compress`, {
        method: "POST",
      ...
      if (!res.ok) {
        throw new Error(data.error || data.detail || "Compression failed");
      }
      setCompressedFile({
        name: name,
        size: data.size,
      });
      setResultUrl(`${API_URL}/api/files/${fileId}/download`);
      
      if (data.size >= originalSize) {
        showToast("PDF is already fully optimized!", "success");
      } else {
  ...
  const uploadFile = useCallback(async (f: File) => {
    ...
    try {
      ...
      setOriginalFile(uploaded);
      
      // Auto-trigger compression
      await compressPdf(uploaded.id, uploaded.name, uploaded.size);
    } catch (err: any) {
      ...
      {(isUploading || isCompressing) && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-white font-semibold text-lg">
            {isUploading ? "Uploading PDF…" : "Compressing PDF…"}
          </p>
          <p className="text-white/60 text-sm">
            {isUploading ? "Getting file details" : "Optimizing PDF and scaling image assets"}
          </p>
        </div>
      )}
      ...
      <header className="sticky top-0 z-40 border-b border-zinc-200/50 dark:border-zinc-900/50 bg-white/70 dark:bg-zinc-950/70 backdrop-blur-md">
        <div className="max-w-5xl mx-auto h-16 px-6 flex items-center gap-4">
          ...
          <span className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 flex items-center gap-2">
            <FileCompressIcon className="w-4 h-4 text-indigo-500" />
            Compress PDF
          </span>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-6 py-14">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 text-xs font-semibold border border-indigo-200/50 dark:border-indigo-900/30 mb-6">
            ...
            Preserve layout, formatting &amp; graphics styles
          </div>
          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-zinc-900 dark:text-white mb-4">
            Compress <span className="text-indigo-600 dark:text-indigo-400">PDF size</span>
          </h1>
          <p className="text-base text-zinc-500 dark:text-zinc-400 max-w-xl mx-auto leading-relaxed">
            Reduce the file size of your PDF documents while completely preserving original document structure, fonts, alignments, and vector elements.
          </p>
        </div>
        ...
        <div className="max-w-xl mx-auto space-y-8">
          ...
          {!resultUrl && (
            <div className="bg-white/60 dark:bg-zinc-950/40 p-5 rounded-2xl border border-zinc-200/50 dark:border-zinc-900/50 backdrop-blur-sm space-y-4">
              <div>
                <p className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 mb-2.5 uppercase tracking-wider text-center">Choose Compression Level</p>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { id: "less", label: "Less", desc: "High quality" },
                    { id: "recommended", label: "Recommended", desc: "Good quality" },
                    { id: "extreme", label: "Extreme", desc: "Lower quality" },
                  ].map((level) => (
                    <button
                      key={level.id}
                      onClick={() => setCompressionLevel(level.id as any)}
                      className={`py-3 px-2 rounded-xl transition-all text-center cursor-pointer flex flex-col items-center justify-center border ${
                        compressionLevel === level.id
                          ? "bg-indigo-655 border-indigo-655 text-white shadow-md shadow-indigo-600/10"
                          : "border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-900 text-zinc-650 dark:text-zinc-400 bg-white/40 dark:bg-zinc-900/10"
                      }`}
                    >
                      <span className="text-xs font-bold">{level.label}</span>
                      <span className={`text-[10px] mt-0.5 ${compressionLevel === level.id ? "text-indigo-200" : "text-zinc-400 dark:text-zinc-500"}`}>{level.desc}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="p-8 rounded-3xl bg-white/60 dark:bg-zinc-950/40 border border-zinc-200/50 dark:border-zinc-900/50 backdrop-blur-sm shadow-xl space-y-6">
              <div className="flex items-center gap-4 pb-6 border-b border-zinc-200/50 dark:border-zinc-900/50">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-white mb-0.5">Compression Completed!</h3>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  { label: "Original Size", value: originalFile ? formatBytes(originalFile.size) : "" },
                  { label: "New Size", value: compressedFile ? formatBytes(compressedFile.size) : "" },
                  { label: "Reduction", value: `${getPercentReduction()}% smaller`, highlight: true },
                ].map((item) => (
                  <div key={item.label} className="rounded-xl bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-200/50 dark:border-zinc-800/50 p-3.5 text-center">
                    <p className="text-[10px] font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider mb-1">{item.label}</p>
                    <p className={`text-sm font-bold ${item.highlight ? "text-emerald-600 dark:text-emerald-400" : "text-zinc-900 dark:text-white"}`}>{item.value}</p>
                  </div>
                ))}
              </div>

              {/* Already optimized info message */}
              {originalFile && compressedFile && compressedFile.size >= originalFile.size && (
                <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/40 text-amber-850 dark:text-amber-300 text-[11px] leading-relaxed text-center font-semibold">
                  💡 Your PDF is already highly optimized. We preserved the original file to prevent size increase and maintain quality.
                </div>
              )}
              ...
              <div className="space-y-3">
                <a
                  href={resultUrl}
                  download={compressedFile?.name.replace(/\.pdf$/i, "") + "_compressed.pdf"}
                ...
                {originalFile && (
                  <Link
                    href={`/preview/${originalFile.id}`}
                    className="w-full py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-900 text-zinc-700 dark:text-zinc-355 text-xs font-semibold transition-all flex items-center justify-center gap-1.5"
                  >
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { icon: FileCompressIcon, label: "Compact File Size" },
              { icon: SparklesIcon, label: "Perfect Layout" },
              { icon: ShieldCheckIcon, label: "High Quality" },
            ].map((f) => {
              const Icon = f.icon;
              return (
                <div key={f.label} className="flex flex-col items-center gap-2.5 py-3.5 px-2 rounded-xl bg-white/60 dark:bg-zinc-900/40 border border-zinc-200/50 dark:border-zinc-800/50 text-center">
                  ...
                  <span className="text-[11px] font-bold text-zinc-655 dark:text-zinc-400">{f.label}</span>
                </div>
              );
            })}
          </div>
            ...
            {[
              {
                step: "01",
                title: "Upload PDF",
                desc: "Drop or choose the PDF document you want to optimize.",
                icon: UploadIcon,
              },
              {
                step: "02",
                title: "Select Settings",
                desc: "Choose Less, Recommended, Extreme or target a specific file size.",
                icon: SettingsIcon,
              },
              {
                step: "03",
                title: "Download PDF",
                desc: "Download your compressed PDF file with reduced size.",
                icon: DownloadIcon,
              },
            ].map((s) => {
              const Icon = s.icon;
              return (
                <div key={s.step} className="p-5 rounded-2xl bg-white/40 dark:bg-zinc-900/20 border border-zinc-200/50 dark:border-zinc-800/50 flex flex-col items-start">
                  ...
                  <p className="text-[11px] text-zinc-550 dark:text-indigo-400 leading-normal">{s.desc}</p>
                </div>
              );
            })}
          </div>
