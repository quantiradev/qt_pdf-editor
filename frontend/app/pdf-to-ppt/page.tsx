...
const FilePptIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg fill="currentColor" viewBox="0 0 24 24" {...props}>
    <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-2 10h-4v2h4v2h-4v2h-2V7h6v2h-4v2h4v2z" />
  </svg>
);
...
export default function PdfToPptPage() {
  ...
  const convertFile = async (uploadedFile: { id: string; name: string }, password?: string) => {
    ...
    setError(null);
    try {
      const url = `${API_URL}/api/files/${uploadedFile.id}/pdf-to-ppt` + (password ? `?password=${encodeURIComponent(password)}` : "");
      ...
      const newName = uploadedFile.name.replace(/\.pdf$/i, "") + ".pptx";
      setResultUrl(`${API_URL}/api/files/${uploadedFile.id}/download-ppt`);
      setResultName(newName);
      showToast("Conversion successful! Your PowerPoint presentation is ready.", "success");
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 font-sans relative overflow-x-hidden">
      ...
      {(isUploading || isConverting) && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-white font-semibold text-lg">
            {isUploading ? "Uploading PDF…" : "Converting PDF to PowerPoint…"}
          </p>
          <p className="text-white/60 text-sm">
            {isUploading ? "Getting file details" : "Generating high-quality presentation slides"}
          </p>
        </div>
      )}
      ...
      <header className="sticky top-0 z-40 border-b border-zinc-200/50 dark:border-zinc-900/50 bg-white/70 dark:bg-zinc-950/70 backdrop-blur-md">
        <div className="max-w-5xl mx-auto h-16 px-6 flex items-center gap-4">
          ...
          <span className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 flex items-center gap-2">
            <FilePptIcon className="w-4 h-4 text-indigo-500" />
            PDF to PowerPoint
          </span>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-6 py-14">
        ...
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-indigo-50 dark:bg-indigo-950/40 text-indigo-750 dark:text-indigo-300 text-xs font-semibold border border-indigo-200/50 dark:border-indigo-900/30 mb-6">
            <SparklesIcon className="w-3.5 h-3.5 text-indigo-500" />
            Advanced Page Layout Preservation
          </div>
          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-zinc-900 dark:text-white mb-4">
            Convert <span className="text-indigo-650 dark:text-indigo-400">PDF to PowerPoint</span>
          </h1>
          <p className="text-base text-zinc-500 dark:text-zinc-400 max-w-xl mx-auto leading-relaxed">
            Reconstruct your PDF documents into fully editable PowerPoint (.pptx) presentations. Every page will be perfectly framed as a high-fidelity slide.
          </p>
        </div>
        ...
        <div className="max-w-xl mx-auto space-y-8">
            ...
            <div
              ...
              className={`relative w-full py-20 px-8 rounded-3xl border-2 border-dashed cursor-pointer transition-all duration-300 select-none text-center group
                ...
                ${isDragging
                  ...
                  : "border-zinc-300 dark:border-zinc-800 hover:border-indigo-400 dark:hover:border-indigo-650"
                }`}
            >
            ...
            <div className="p-8 rounded-3xl bg-white/60 dark:bg-zinc-950/40 border border-zinc-200/50 dark:border-zinc-900/50 backdrop-blur-sm shadow-xl space-y-6">
              <div className="flex items-center gap-4 pb-6 border-b border-zinc-200/50 dark:border-zinc-900/50">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-zinc-555 dark:text-zinc-400 truncate max-w-[320px]">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { icon: FilePptIcon, label: "PowerPoint (.pptx)" },
              { icon: SparklesIcon, label: "Preserve Aspect" },
              ...
              return (
                <div key={f.label} className="flex flex-col items-center gap-2.5 py-3.5 px-2 rounded-xl bg-white/60 dark:bg-zinc-900/40 border border-zinc-200/50 dark:border-zinc-800/50 text-center">
                  <div className="text-indigo-650 dark:text-indigo-450">
            ...
            {[
              {
                step: "01",
                title: "Upload PDF",
                desc: "Drop or choose the PDF presentation you want to convert.",
                icon: UploadIcon,
              },
              {
                step: "02",
                title: "Processing",
                desc: "Our engine scales slides and imports page components.",
                icon: SettingsIcon,
              },
              {
                step: "03",
                title: "Download PPTX",
                desc: "Download the converted PowerPoint slides instantly.",
                icon: DownloadIcon,
              },
            ].map((s) => {
              const Icon = s.icon;
              return (
                <div key={s.step} className="p-5 rounded-2xl bg-white/40 dark:bg-zinc-900/20 border border-zinc-200/50 dark:border-zinc-800/50 flex flex-col items-start">
                  ...
                  <div className="text-xs font-black text-indigo-550 dark:text-indigo-450 uppercase tracking-widest mb-1.5">{s.step}</div>
          ...
          <div className="w-full max-w-md bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 shadow-2xl space-y-6 relative overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handlePasswordSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm transition-all shadow-lg shadow-indigo-600/20"
                >
                  Convert PDF
                </button>
              </div>
