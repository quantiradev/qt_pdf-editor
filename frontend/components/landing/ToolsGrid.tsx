...
export default function ToolsGrid({
  ...
  const tools: PDFTool[] = [
    ...
    {
      name: "Compare PDF",
      description: "Compare two PDF versions side-by-side to highlight text additions, removals, and modifications.",
      category: "edit",
      badge: "New" as const,
      href: "/tools/compare",
      icon: (
    ...
    },
    {
      ...
      description: "Convert PDF presentations to editable Microsoft PowerPoint slide decks.",
      category: "convert",
      href: "/pdf-to-ppt",
      icon: (
        <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
          ...
          {filteredTools.map((tool, idx) =>
            tool.href ? (
              <Link
                key={idx}
                href={tool.href}
                className="block p-5 rounded-2xl border border-zinc-200 dark:border-zinc-900 bg-white/60 dark:bg-zinc-950/20 hover:border-indigo-300 dark:hover:border-indigo-700 hover:shadow-lg hover:shadow-indigo-500/5 transition-all duration-300 group cursor-pointer"
              >
              <div>
            ...
            <div
              key={idx}
              onClick={onAction}
              className="p-5 rounded-2xl border border-zinc-200 dark:border-zinc-900 bg-white/60 dark:bg-zinc-950/20 hover:border-indigo-300 dark:hover:border-indigo-700 hover:shadow-lg hover:shadow-indigo-500/5 transition-all duration-300 group cursor-pointer"
            >
              <div>
                <div className="flex items-center justify-between mb-5">
                  {/* Tool Icon */}
